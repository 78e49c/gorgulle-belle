...
fun getCommonQuestionPrompts(): List<Message> {
    val topicSpecificPrompt = listOf(
        Message("system", "Generate a question based on the concept provided above." +
                ...
                "The topic of the question should be $questionTopic. " +
                "Do not add any explanations or embellishments, your response should only be in the required format.")
    )
    return topicSpecificPrompt + formatPrompt
}

fun getQuestionPrompts(type: Int): List<Message> {
    ...
    return when (type) {
        0 -> konuTespitiPrompts
        1 -> gereklilikTespitiPrompts
        2 -> uygunEylemPrompts
        else -> emptyList()
    }
}
